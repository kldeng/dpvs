...
#define VXLAN_VNI_MIN   1
#define VXLAN_VNI_MAX   16 * 1024 * 1024
          ...
          unsigned int *options, unsigned int *format)
{
    ...
    struct poptOption options_table[] = {
        ...
        { "nattunnel" , 'N' , POPT_ARG_NONE, NULL, 'N', NULL, NULL },
        { "weight", 'w', POPT_ARG_STRING, &optarg, 'w', NULL, NULL },
        { "rs-vni", '\0', POPT_ARG_STRING, &optarg, TAG_RS_VNI, NULL, NULL },
        { "vip-vni", '\0', POPT_ARG_STRING, &optarg, TAG_VIP_VNI, NULL, NULL },
        { "hip", 'Y', POPT_ARG_STRING, &optarg, 'Y', NULL, NULL },
        { "mac", 'O', POPT_ARG_STRING, &optarg, 'O', NULL, NULL },
    ...
    while ((c=poptGetNextOpt(context)) >= 0){
        ...
        case 'r':
            ...
            break;
            case TAG_RS_VNI:
                    set_option(options, OPT_VNI);
                    if ((ce->dest.vx_vni_rs = string_to_number(optarg, VXLAN_VNI_MIN, VXLAN_VNI_MAX)) == -1)
                        fail(2, "illegal rs vni specified");
                    break;
        case TAG_VIP_VNI:
            set_option(options, OPT_VIP_VNI);
                    if ((ce->svc.vx_vni_vip = string_to_number(optarg, VXLAN_VNI_MIN, VXLAN_VNI_MAX)) == -1)
                fail(2, "illegal vip vni specified");
            printf("vip-vni = %u\n", ce->svc.vx_vni_vip);
            break;
            case 'Y':
                set_option(options, OPT_HIP);
                    struct in_addr sip;
            //Note: only support IPv4 now
            if (inet_pton(AF_INET, optarg, &sip) > 0 || host_to_addr(optarg, &sip) != -1) { 
                ce->dest.af_hip = AF_INET;
                ce->dest.addr_hip.in = sip;

                ce->dest.hip = sip.s_addr;
            } else {
                fail(2, "illegal hip specified");
            }
                    break;
        case 'O':
            set_option(options, OPT_MAC);
            struct ether_addr *p;
            if (!(p = ether_aton(optarg))) 
                fail(2, "illegal mac specified");
            ce->dest.mac = *p;
            //memcpy( &ce->dest.mac, p, sizeof(struct ether_addr));
            break;
        case 'i':
        ...
        case 'J':
            ...
            case 'N':
                    set_option(options, OPT_FORWARD);
                    ce->dest.conn_flags = IP_VS_CONN_F_NAT_TUNNEL;
                break;
        case 'm':
            set_option(options, OPT_FORWARD);
...
static void print_title(unsigned int format)
{
    if (format & FMT_STATS)
        ...
        printf("Prot LocalAddress:Port VNI Scheduler Flags\n"
               "  -> RemoteAddress:Port   Forward  VNI  MAC  HIP   Weight ActiveConn InActConn\n");
}
...
print_service_entry(ipvs_service_entry_t *se, unsigned int format)
{
    ...
    if (format & FMT_RULE) {
        ...
        printf("%s %u %s", svc_name, se->vx_vni_vip, se->sched_name);
        if (se->flags & IP_VS_SVC_F_PERSISTENT) {
    ...
    for (i = 0; i < d->num_dests; i++) {
        ...
        if (format & FMT_RULE) {
            ...
            if (e->conn_flags == IP_VS_CONN_F_NAT_TUNNEL) {
                char buf[64];
                //printf("  -> %-28s %-7s %-7u %-7s %-7s %-6d %-10u %-10u\n",
                printf("  -> %s %s %u %s %s %d %u %u\n",
                        dname, 
                    fwd_name(e->conn_flags), 
                    e->vx_vni_rs, 
                    ether_ntoa(&e->mac), 
                    inet_ntop(se->af, &e->hip, buf, sizeof(buf)),
                        e->weight, 
                    e->activeconns, 
                    e->inactconns);
            } else {
